...
def volumetric_render(student: StudentModel,
    ...
    for start in range(0, rays_o.shape[0], chunk_size):
        ...
        rgb_map = rgb_map + (1.0 - opacity_map)[..., None] * background_color
...
def render_student_scene(args):
    ...
    for directory in (rgb_dir, depth_dir, opacity_dir):
    ...
    with torch.no_grad():
        for frame_idx, frame in enumerate(tqdm(frames, desc="Rendering student", unit="frame")):
            ...
            rgb = outputs["rgb"].clamp(0.0, 1.0).view(H, W, 3).cpu().numpy()
            ...
            depth_uint8 = (depth_norm.clip(0.0, 1.0) * 255.0).astype(np.uint8)
            imageio.imwrite(depth_dir / file_stub.with_suffix(".png"), depth_uint8)

            opacity_uint8 = (opacity * 255.0).astype(np.uint8)
